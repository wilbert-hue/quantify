...
import { REGIONS_IN_ORDER } from "./template";
 ...
 * - Segment rows from segment uploads, repeated per reference region order
 * - Geography rows from geography uploads only (no template fallback)
 */
export function buildAlignedDataset(parsedRows: ExtractedRow[]): ExtractedRow[] {
  ...
  for (const region of REGIONS_IN_ORDER) {
